...
class ApiService {
  ...
  async getLiveHumidity(id: string): Promise<{ humidity: number; timestamp: number; entityId: string; sensorName: string }> {
    const response = await fetch(`${API_BASE_URL}/projects/${id}/live-humidity`, {
      headers: getAuthHeaders()
    });
    if (!response.ok) {
      const error = await response.json().catch(() => ({}));
      throw new Error(error.error || 'Failed to fetch live humidity');
    }
    return response.json();
  }
