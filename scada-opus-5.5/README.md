# Hakko — Dashboard de supervision des fermentations

Spécification complète pour reconstruire **à l'identique** (design et fonctionnalités) le dashboard Hakko de suivi de fermentations : bière, miso, koji et garum.

> **Consigne pour l'agent**
> Le résultat doit être visuellement et fonctionnellement indiscernable de la référence. Les annexes A, B et C contiennent le code source de référence complet (HTML, CSS, JavaScript). En cas de doute ou de contradiction entre la description et les annexes, **les annexes font foi**. Ne pas « améliorer », ne pas changer de palette, de police, d'espacement ni de libellé. Tous les textes de l'interface sont en français, tels qu'écrits ici.

---

## 1. Livrable et contraintes techniques

- **Un seul fichier** `hakko-dashboard.html`, autonome : HTML, CSS et JavaScript inline, aucun build.
- **JavaScript vanilla** (ES2020+), sans framework ni librairie. Les graphiques sont dessinés à la main en SVG inline.
- **Police** : IBM Plex Sans (400, 500, 600) via Google Fonts, avec le repli `ui-sans-serif, system-ui, -apple-system, 'Segoe UI', Roboto, sans-serif`. Tous les chiffres utilisent `font-variant-numeric: tabular-nums` (classe `.num`).
- **Persistance** dans `localStorage` :
  - `hakko-dashboard-v1` pour tout l'état (recettes, lots, appareils, événements) ;
  - `hakko-theme` pour le thème (`auto` | `light` | `dark`).
  - Chaque accès est entouré de `try/catch`.
- **Données** : les mesures sont **simulées** (modèle déterministe, voir §8), avec un rafraîchissement toutes les **3 s** pour imiter un polling Home Assistant.
- **Routage** par hash (`location.hash`).
- **Langue** `fr`, formats français : virgule décimale, dates `toLocaleDateString('fr-FR')`.
...
```
État global S = { recipes[], batches[], devices[], events[], heatSeeded, stripSeeded }
      │
      ├─ tick()         toutes les 3 s : met à jour les mesures des appareils et la régulation des prises
      ├─ save()         écrit S dans localStorage
      ├─ refreshLive()  met à jour uniquement les éléments marqués data-live / data-state /
      │                 data-spark / data-plug / data-plugbtn / data-chart[data-live2] / #feed / #journal-list
      └─ render()       re-rend la vue complète quand la route change ou après une action utilisateur
```

**Routes**

| Hash | Vue | Onglet actif |
|---|---|---|
| `#/` | Vue d'ensemble | home |
| `#/f/:id` | Détail d'un lot actif (redirige vers `#/archive/:id` si le lot est terminé) | home |
| `#/archive` | Liste des lots archivés | archive |
| `#/archive/:id` | Fiche d'archive en lecture seule | archive |
| `#/recettes` | Liste des recettes | recettes |
| `#/recettes/new` | Création d'une recette (pleine page) | recettes |
| `#/recettes/:id` | Édition d'une recette (pleine page) | recettes |
| `#/appareils` | Appareils | appareils |

`#view` reçoit `data-page="detail"` pour `#/`, `#/f/*` et `#/archive/*`. Ces trois pages sont en mode « un seul écran sans scroll » (voir §9).

Les interactions passent par **délégation d'événements** : `click`, `change`, `input` et `submit` sur `document`, avec des attributs `data-action` et `data-form`. Les lignes de tableau cliquables portent `data-href`.
...
Les jetons sont définis sur `:root`. Le thème sombre s'applique via `@media (prefers-color-scheme: dark)` sur `:root:not([data-theme="light"])`, et via `:root[data-theme="dark"]`.

| Jeton | Clair | Sombre | Usage |
|---|---|---|---|
| `--bg` | `#F4F5F7` | `#0E1116` | Fond de page |
| `--surface` | `#FFFFFF` | `#161A21` | Cartes |
| `--surface2` | `#F7F8FA` | `#1A1F27` | En-têtes de tableau, fonds secondaires |
| `--hover` | `#F1F3F6` | `#1E242D` | Survol |
| `--ink` | `#111827` | `#E6EAF0` | Texte principal |
| `--ink2` | `#374151` | `#C4CAD3` | Texte secondaire |
| `--muted` | `#6B7280` | `#8C95A3` | Libellés |
| `--faint` | `#9CA3AF` | `#6B7380` | Texte discret, lignes min/max |
| `--line` | `#E5E7EB` | `#262C36` | Bordures |
| `--line2` | `#D1D5DB` | `#323A46` | Bordures de champs et boutons |
| `--grid` | `#EEF0F3` | `#1F252E` | Grille des graphiques, pistes de barres |
| `--accent` | `#2350B8` | `#7FA2EC` | Accent (bouton primaire, onglet actif, humidité) |
| `--accent-soft` | `#EAF0FC` | `#1C2740` | Halo de focus |
| `--ok` / `--ok-bg` | `#15803D` / `#E7F6EC` | `#5CC486` / `#142A1D` | Dans la cible, en ligne |
| `--warn` / `--warn-bg` | `#B45309` / `#FDF3E1` | `#E8A955` / `#2E2412` | Écart léger, chauffe on |
| `--bad` / `--bad-bg` | `#B91C1C` / `#FDECEC` | `#F07F7F` / `#321818` | Hors plage, hors ligne |
| `--biere` | `#C98410` | `#E3A53A` | Type Bière |
| `--miso` | `#95602F` | `#C98E5A` | Type Miso |
| `--koji` | `#5A8A2E` | `#8DBE5C` | Type Koji |
| `--garum` | `#9B2C4A` | `#D66E8A` | Type Garum |
| `--side` | `#151A21` | `#0A0C10` | Barre latérale (sombre dans les deux thèmes) |
| `--side-ink` / `--side-ink2` | `#AEB6C2` / `#F3F5F8` | idem | Texte de la barre latérale |
| `--side-active` / `--side-line` | `#232A34` / `#262D38` | `#1B2029` / `#1C212A` | Élément actif, séparateurs |
| `--shadow` | `0 1px 2px rgba(16,24,40,.05)` | `none` | Ombre des cartes et boutons |

Deux couleurs sont **en dur** pour les boutons de la multiprise : ON `#1F9D55`, OFF `#C53030`, texte blanc.

### 3.2 Typographie

- Texte de base : 14 px, interligne 1.5.
- Titres de page (`.ph h1`) : 22 px, 600.
- Titre des pages en mode écran unique (`.dp-title h1`) : 21 px, 600.
- En-têtes de carte (`.card-h h2`) : 14 px, 600.
- Valeurs KPI : 26 px (20 px en mode compact), 600, letter-spacing −0.02em. L'unité est en `<small>` à 14 px, 500, couleur `--muted`.
- Libellés : 12,5 à 13 px, couleur `--muted`, en casse normale. **Jamais de majuscules forcées.**

### 3.3 Formes et espacements

- Rayons : cartes 10 px ; boutons et champs 7 px ; badges 5 px ; tuiles de lot et de prise 8 à 10 px ; boîte de dialogue 12 px.
- Hauteurs : boutons 36 px (`.sm` : 30 px) ; champs 36 px ; onglets 44 px (42 px dans le graphique du détail) ; barre haute 60 px.
- Padding : `.card-h` 14 × 18 px (10 × 16 px en mode écran unique) ; `.card-b` 18 px ; contenu de page 28 × 32 px (bas 64 px) ; en mode écran unique, 20 px en haut et en bas.

### 3.4 Composants

- **`.btn`** : fond `--surface`, bordure `--line2`, ombre `--shadow`, poids 500. Variantes :
  - `.primary` : fond `--accent`, texte blanc (texte `#0E1116` en sombre) ;
  - `.ghost` : sans bordure ni fond ;
  - `.danger` : texte `--bad`.
- **`.card`** avec `.card-h` (titre à gauche, action à droite, bordure basse) et `.card-b`. Un compteur à côté du titre utilise `.sub`.
- **`.badge`** : 22 px de haut. Variantes `ok`, `warn`, `bad` (fond teinté, sans bordure) et `acc` (accent). Un point `.dot` de 6 px peut le précéder.
- **`.type`** : carré coloré de 8 × 8 px (rayon 2), couleur `--c`, suivi du libellé du type.
- **`.tabs`** : onglets soulignés 2 px `--accent` quand `aria-selected="true"`. Un compteur `.cnt` peut suivre le libellé.
- **`.seg`** : contrôle segmenté bordé, bouton actif `aria-pressed="true"` sur fond `--surface2`.
- **`.kpi`** : libellé avec icône, valeur, sous-texte (`.s.ok`, `.s.warn`, `.s.bad`).
- **`table.data`** : en-tête sur fond `--surface2`, cellules 12 × 16 px. Avec `.stack`, sous 900 px, le tableau passe en fiches empilées dont chaque cellule affiche son `data-l` comme libellé.
- **`.switch`** : interrupteur 36 × 20 px, vert quand il est actif.
- **`.stepper`** : boutons − et + autour d'un champ numérique centré, 18 px, 600.
- **`.strip`** : frise de progression. Cases carrées de rayon 2, écart 3 px ; `.past` pleine de la couleur du type, `.now` à 45 % d'opacité avec un contour.
- **`.bar`** : barre de progression de 6 px.
- **`.outlet`** : tuile d'une prise de la multiprise (voir §6.6).
- **Icônes** : SVG 24 × 24, trait `currentColor` de 1.8, extrémités rondes, affichés en 16 px (classe `.i`).
- **Logo** : pictogramme de bocal (voir `MARK` dans l'annexe C), dans un carré blanc de 28 px à rayon 7.

---

## 4. Coquille de l'application

- **Barre latérale** de 236 px, fond `--side`, collante sur toute la hauteur :
  - Logo et « Hakko », sous-titre « Supervision fermentation ».
  - **Suivi** : Vue d'ensemble (nombre de lots en cours), Archive (nombre de lots terminés).
  - **Bibliothèque** : Recettes (nombre de recettes actives).
  - **Installation** : Appareils (nombre total d'appareils).
  - Pied : point vert pulsé, « Home Assistant », « Dernière synchro HH:MM:SS » mis à jour à chaque tick.
- **Barre haute** de 60 px, collante, fond translucide avec flou. On y trouve le fil d'Ariane (liens séparés par `/`, dernier élément en gras), puis à droite « En direct, HH:MM:SS » et le bouton de thème.
  - Le bouton de thème fait tourner `auto` → `light` → `dark`, avec les icônes demi-disque, soleil et lune.
- **Sous 900 px** : la barre latérale est masquée, le logo apparaît dans la barre haute, et une **barre d'onglets fixe en bas** affiche Accueil, Archive, Recettes et Appareils (actif en `--accent`).

---

## 5. Données

### 5.1 Types de fermentation

| Clé | Libellé | Couleur | Tolérance `tol` |
|---|---|---|---|
| `biere` | Bière | `--biere` | 1.0 |
| `miso` | Miso | `--miso` | 4.0 |
| `koji` | Koji | `--koji` | 2.5 |
| `garum` | Garum | `--garum` | 1.5 |

La plage « dans la cible » vaut consigne ± `tol/2`. L'hystérésis de chauffe `HYST` vaut **0,2 °C**.

### 5.2 Schémas

```ts
Recipe  { id, name, type, desc, duration /* jours, 0.5 min */, temp /* °C */,
          og?, fg?, volume? /* bière */, hum? /* koji, % HR */, salt? /* miso, garum, % */,
          ingredients: {n, q}[], steps: string[], archived: boolean }

Batch   { id, name, recipeId, recipe: Recipe /* copie figée au lancement */,
          start, end?, status: 'active' | 'done',
          gravity?: [timestamp, density][], notes: {t, x}[],
          targets?: [timestamp, temp][]  /* historique des consignes */,
          devSnap?: {name, kind}[]       /* appareils reliés au moment de la clôture */ }

Device  { id, name, kind: 'temp' | 'hum' | 'plug', batch: batchId | null, online,
          seen?, value?, battery? /* sondes */,
          mode?: 'auto' | 'manuel', on?, power?, slot?: 1..5 /* prises de la multiprise */ }

Event   { t, k: 'heat-on' | 'heat-off' | 'note' | 'alert' | 'batch' | 'target', x, b,
          tp?, tg?, manual?, from?, to? }
```

### 5.3 Données initiales

Reprendre exactement la fonction `seed()` de l'annexe C :

- **6 recettes** :
  - actives : Saison du Nord, Miso d'orge, Koji de riz, Garum de champignons ;
  - archivées : IPA Citra, Miso cacao-épeautre.
- **5 lots** :
  - b1 « Saison du Nord #3 », démarré il y a 9,3 j, avec 6 relevés de densité ;
  - b2 « Miso d'orge 2026 », il y a 142 j ;
  - b3 « Koji de riz — amazake », il y a 26 h ;
  - b4 « Garum de champignons », il y a 20 j ;
  - b0 « IPA Citra #2 », terminé.
- **9 appareils** d'origine, dont « Sonde cave » hors ligne avec 8 % de batterie.
- **Migrations au démarrage** :
  - `stripSeeded` : les 3 prises existantes deviennent les emplacements 1 à 3 de la multiprise, et l'on crée « Prise 4 » et « Prise 5 » (libres, mode manuel) ;
  - `heatSeeded` : on reconstruit l'historique de chauffe des 36 dernières heures par simulation ;
  - on ajoute un `devSnap` aux lots terminés qui n'en ont pas.

---

## 6. Pages

### 6.1 Vue d'ensemble (`#/`), un seul écran

```
[Vue d'ensemble  Situation au <jour date>]                  [Appareils] [+ Lancer une fermentation]
[KPI Lots en cours][KPI Écarts de température][KPI Appareils en ligne][KPI Chauffe active]
┌ Lots en cours n ────────────── Voir les recettes ┐ ┌ Activité récente ─────────┐
│ grille de tuiles de lot, 2 colonnes (min 250 px) │ │ 30 derniers événements    │
│ scroll interne au-delà de ce qui tient           │ │ scroll interne (≈60 %)    │
│                                                  │ ├ État des appareils ─Tout voir┐
│                                                  │ │ 5 appareils, pires en tête (≈40 %) │
└──────────────────────────────────────────────────┘ └───────────────────────────┘
```

- **KPI** :
  - Lots en cours, avec « n recettes disponibles » ;
  - Écarts de température : nombre de lots en `warn` ou `bad`, sous-texte « À vérifier » ou « Tous les lots dans leur plage » ;
  - Appareils en ligne `x/y`, sous-texte « n hors ligne » ou « Tous connectés » ;
  - Chauffe active : n prises et « W consommés ».
- **Tuile de lot** (lien vers `#/f/:id`, liseré de la couleur du type) :
  - en-tête : type et badge d'état (Dans la cible / Écart léger / Hors plage / Sans sonde) ;
  - nom, puis « Démarrée le <date>, <durée> prévus » ;
  - une grille de 3 cellules séparées par des traits : Température (en direct), Cible, et une 3ᵉ cellule qui affiche l'Humidité pour le koji, la Densité pour une bière avec relevés, ou la Chauffe (Active / Veille / —) sinon ;
  - une courbe des dernières 24 h avec la consigne en pointillé ;
  - le libellé de progression, le pourcentage et une barre.
- **Événement** : icône dans un carré de 28 px (flamme, crayon, alerte ou lecture), texte, puis « il y a … ».
- Les lots terminés n'apparaissent **pas** ici.

### 6.2 Détail d'un lot actif (`#/f/:id`), un seul écran

```
[Nom  ▪Type  (badge)  Démarrée le …]                                      [Terminer le lot]
[KPI Température][KPI Consigne][KPI Humidité | Densité + Alcool estimé | Salinité][KPI Chauffage]
┌ Température | Humidité | Densité ─────────── [24 h|7 j|Tout] ┐ ┌ Consigne ─ Actuelle x °C ┐
│ graphique qui remplit l'espace, plage tolérée, consigne      │ │ [− 24 °C +] [Appliquer]   │
│ légende                                                      │ │ Chauffe active sous …     │
├ Journal [Tout|Notes|Chauffe|Consigne] ┬ Appareils reliés ────┤ ├ Progression ───── 44 % ──┤
│ liste avec scroll interne             │ table avec scroll    │ │ Jour 10 sur 21, frise     │
│ [Ajouter une observation] [Ajouter]   │                      │ │ Début / Restant / Fin     │
└───────────────────────────────────────┴──────────────────────┘ ├ Paramètres (scroll) ──────┤
```

- Grille : colonnes `1.7fr 1fr 320px`, rangées `1.3fr 1fr`, écart 14 px.
- **Onglets du graphique** : Température (toujours) ; Humidité (koji) ; Densité (bière). La période (24 h, 7 j, Tout) est masquée sur l'onglet Densité ; un lot de moins de 3 jours ne propose que 24 h et Tout.
- **Onglet Densité** : le graphique est à gauche et un panneau de 340 px à droite contient :
  - un formulaire avec « Densité mesurée » (pas 0,001) et « Date de la mesure » (`datetime-local`, préremplie avec l'instant présent, bornée entre le début du lot et maintenant), puis le bouton « Ajouter le relevé » ;
  - un tableau des relevés (Date, Densité, Alcool), triés du plus récent au plus ancien, chacun avec un bouton ✕ pour le supprimer.
- **Consigne** : un stepper par pas de 0,5 et le bouton « Appliquer ».
  - L'application ajoute `[now, v]` à `targets` (initialisé à `[[start, ancienne]]`), met à jour `recipe.temp`, journalise un événement `target` et exécute `tick()`.
  - La température simulée rejoint la nouvelle consigne progressivement (τ = 40 min).
- **Journal** : il fusionne les notes du lot et les événements `heat-on`/`heat-off` (ceux qui ont `tp`) et `target`, triés du plus récent au plus ancien, 150 entrées au maximum. Chaque ligne est une grille `auto | 80px | 1fr` :
  - Chauffe : badge « Chauffe on » (`warn`) ou « Chauffe off », puis **`57,0 °C` (−3,0 °C) - Activation de la chauffe** ou **`65,0 °C` (+5,0 °C) - Désactivation de la chauffe**, suivi de « , manuelle » si c'était manuel. L'écart vaut `tp − tg`, avec un signe `+` ou `−` (vrai signe moins).
  - Consigne : badge « Consigne » (`acc`), puis « Consigne modifiée de 60,0 °C à **58,0 °C** ».
  - Note : badge « Note », puis le texte.
- **Composition** (ingrédients et étapes) : **absente** de cette page.
- **Terminer le lot** : après confirmation (« Terminer ce lot ? Il sera déplacé dans l'Archive et les appareils reliés seront libérés. »), on passe `status` à `done`, on renseigne `end`, on enregistre `devSnap`, on détache les appareils (les prises sont éteintes) et on navigue vers `#/archive/:id`.

### 6.3 Archive (`#/archive`)

- Titre « Archive », sous-titre « Lots terminés, consultables en lecture seule ».
- Barre d'outils : recherche et filtre segmenté par type.
- Tableau : Lot (avec le nom de la recette dessous), Type, Période « <début> au <fin> », Durée, Temp. moyenne « x °C / consigne », Dans la plage (badge `ok` ≥ 90 %, `warn` ≥ 70 %, `bad` sinon), Résultat.

### 6.4 Fiche d'archive (`#/archive/:id`), lecture seule, un seul écran

- Aucun bouton d'action ni formulaire. L'en-tête montre le nom, le type, un badge « Archivé » et « Du … au … ».
- **KPI** : Durée totale (face à la durée prévue), Température moyenne (et écart-type), Min / max (et amplitude), Dans la plage (%), puis selon le cas :
  - Densité finale (« x % vol., att. y % ») ;
  - Humidité moyenne ;
  - Chauffe (heures, nombre d'activations, % du temps).
- **Grille** identique à celle du détail :
  - « Température sur toute la durée » : moyenne et courbes min/max en `--faint`, consigne finale, plage tolérée ;
  - second graphique : Densité (bière), Humidité relative (koji), ou « Températures par jour / par semaine / par tranche de 4 h » (moyenne, min et max) ;
  - Journal figé ;
  - à droite, « Résumé de la recette » avec scroll : description, paramètres, dates, appareils utilisés, ingrédients, étapes.

### 6.5 Recettes

- **Liste** : onglets Actives / Archivées avec compteurs, recherche, filtre par type. Tableau : Nom (avec description tronquée), Type, Durée, Température, Paramètre clé (`OG → FG`, `x % HR` ou `x % sel`), Lancements, Dernier lancement. Un clic sur une ligne ouvre l'édition.
- **Formulaire** (pleine page, avec « Annuler » et « Enregistrer » dans l'en-tête) :
  - à gauche, les cartes :
    - Informations générales : 4 tuiles radio de type, Nom, Description ;
    - Paramètres de fermentation : Durée (jours) et Température (°C) ; OG, FG et Volume pour la bière ; Humidité pour le koji ; Salinité pour le miso et le garum. Les champs s'affichent selon `form[data-type]` ;
    - Ingrédients : lignes Ingrédient / Quantité / ✕, et « Ajouter un ingrédient » ;
    - Étapes : lignes numérotées, et « Ajouter une étape ».
  - à droite, un panneau collant :
    - pour une recette existante : « Lancer une fermentation » (si elle n'est pas archivée), « Archiver » ou « Restaurer », et l'Historique des lots ;
    - pour une nouvelle recette : une aide.
  - Validation : nom obligatoire, durée > 0, température renseignée. Les erreurs s'affichent dans `#ferr`.
  - **Lancer une fermentation** crée un lot nommé `<recette> #n` avec une copie profonde de la recette et, pour une bière, un premier relevé égal à l'OG. On navigue ensuite vers son détail.

### 6.6 Appareils

- **KPI** : En ligne `x/y`, Batterie faible (< 20 %), Prises allumées (sur n), Puissance instantanée en W.
- **Onglets** avec compteurs : Tous, Température, Humidité, Prises.
- **Tous, Température, Humidité** : tableau avec les colonnes Appareil (icône, nom, identifiant ou « Multiprise, prise n »), Type, Mesure (en direct), Fermentation (sélecteur), Batterie (barre verte, orange sous 50 % ou rouge sous 20 %, ou « Secteur »), Dernière remontée, État (badge), Contrôle (mode, interrupteur et bouton ✕).
- **Prises : la multiprise.**
  - En-tête : « Multiprise connectée », avec « 5 prises, allumées : n, puissance : x W ».
  - **5 tuiles** (5 colonnes, 3 sous 1250 px, 2 sous 760 px, 1 sous 440 px). Chaque tuile contient :
    - « Prise n » avec une LED (rouge éteinte, verte allumée) et la puissance ;
    - le nom de l'appareil ;
    - le sélecteur « Fermentation » ;
    - le sélecteur « Mode », avec « Auto (consigne) » (désactivé si la prise n'est reliée à rien) et « Manuel » ;
    - un **gros bouton de 44 px** : **ON** sur fond vert `#1F9D55` ou **OFF** sur fond rouge `#C53030`, en gras avec letter-spacing 0.06em.
  - Un clic sur le bouton inverse l'état, passe la prise en mode manuel et journalise l'événement. La tuile allumée a une bordure verte. En mode auto, le bouton se met à jour tout seul à chaque tick.
- **Ajouter un appareil** : une `<dialog>` modale avec Nom, Type et Relier à. Une nouvelle prise prend le premier emplacement libre de la multiprise.

---

## 7. Formats

- `fmt(n, d)` : `toFixed(d)` avec une virgule ; `—` si la valeur est absente.
- `fmtDur(j)` : moins de 3 j → « n h » ; moins de 60 j → « n jours » ; 330 j ou plus → « n an(s) » ; sinon « n mois ».
- **Progression** :
  - moins de 3 j : « X h sur Y h » ;
  - plus de 90 j : « Semaine X sur Y » ;
  - sinon : « Jour X sur Y ».
- **Unité de la frise** : 4 h (moins de 3 j), 1 semaine (plus de 90 j), sinon 1 jour.
- `ago(t)` : « il y a n s », « n min », « n h » ou « n j ».

---

## 8. Logique et simulation (voir l'annexe C pour le code exact)

- **`model(b, t)`** : température simulée égale à la consigne effective `tgt(b, t)`, plus une somme de sinusoïdes déterministes (graine = hash de l'id du lot), plus un profil selon le type :
  - bière : pic exothermique d'environ +1,4 °C vers J2 ;
  - miso : saisonnalité annuelle et cycle jour/nuit ;
  - koji : sigmoïde de montée vers 22 h, et humidité décroissante ;
  - garum : bruit réduit.
- **`tgt(b, t)`** : rapproche exponentiellement la température de chaque nouvelle consigne (τ = 40 min).
- **`setAt(b, t)`** : consigne en vigueur à l'instant `t`, sans lissage. Sert au calcul du « dans la plage ».
- **`tick()`** :
  - chaque appareil en ligne prend sa valeur (modèle du lot relié, plus un jitter) ;
  - chaque prise en mode auto s'allume si `tp < consigne − 0,2` et s'éteint si `tp > consigne + 0,2`. Chaque changement appelle `logHeat()` (événement avec `tp`, `tg`) ;
  - puissance : 160 W allumée (410 W pour l'étuve), 0,4 W éteinte.
- **`status(b)`** :
  - |écart| ≤ tol/2 → Dans la cible ;
  - ≤ tol → Écart léger ;
  - au-delà → Hors plage ;
  - sans sonde → Sans sonde.
- **`archStats(b)`** : 161 points sur la durée du lot, chacun moyennant 40 sous-échantillons (moyenne, min, max). On en tire la moyenne, l'écart-type, le min et le max, le % de temps dans la plage, l'humidité moyenne, une simulation de chauffe (activations, heures, % du temps) et des agrégats par jour, semaine ou tranche de 4 h. Les résultats sont mis en cache.
- **Graphiques `drawChart(el)`** :
  - SVG inline, marges L46 R14 T14 B28 ;
  - graduations « nice » (1, 2, 5) ;
  - 4 étiquettes en X ;
  - plage tolérée en rectangle teinté à 9 % ;
  - consigne en pointillé 4-4 avec son étiquette ;
  - surface à 7 % d'opacité ;
  - point sur la dernière valeur en direct ;
  - réticule et infobulle au survol ;
  - `.chart.fill` : l'`svg` est positionné en absolu et redimensionné par `ResizeObserver`.

---

## 9. Mise en page « un seul écran » et responsive

- À partir de 1101 px de large et 640 px de haut, `.dp` (Vue d'ensemble, Détail, Fiche d'archive) mesure `100dvh − 60px − 40px − safe areas` (minimum 600 px). Le contenu interne est en flex/grid avec `min-height: 0`.
- Seuls les blocs longs défilent (`.scroll`, barre fine) : lots, activité, état des appareils, journal, appareils reliés, paramètres, relevés de densité, résumé de recette. **La page elle-même ne défile jamais.**
- Sous 1100 px : une seule colonne avec un scroll normal. Le graphique fait 380 px de haut et le journal 420 px.
- Sous 900 px : barre d'onglets en bas, tableaux `.stack` en fiches.
- Zones de sécurité : `viewport-fit=cover`, et `:root` avec `padding-top` et `padding-bottom` en `env(safe-area-inset-*)`.

---

## 10. Checklist d'acceptation

- [ ] Les 4 pages principales et les 4 sous-pages s'affichent sans erreur console, en clair comme en sombre.
- [ ] La Vue d'ensemble, le Détail et la Fiche d'archive tiennent sans scroll en 1440 × 860 et en 1280 × 720.
- [ ] Les valeurs en direct changent toutes les 3 s sans perdre le focus ni la position de scroll.
- [ ] Un changement de consigne produit une entrée « Consigne » puis, selon le sens, une entrée `x °C (±y °C) - Activation/Désactivation de la chauffe`.
- [ ] Un relevé de densité ajouté avec une date passée se place à cette date, dans le graphique et dans le tableau.
- [ ] « Terminer le lot » retire le lot de l'accueil et ouvre sa fiche d'archive, en lecture seule.
- [ ] Les 5 prises de la multiprise affichent un bouton ON vert ou OFF rouge qui bascule au clic.
- [ ] L'état est conservé après un rechargement de la page (`localStorage`).

---

## Annexe A — Structure HTML (`<body>`)

```html
<body>
<div class="app">
  <aside class="side">
    <a class="brand" href="#/"><span class="brand-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M8 3h8M9 3v3.2C6.6 7.2 5 9.4 5 12v6a3 3 0 0 0 3 3h8a3 3 0 0 0 3-3v-6c0-2.6-1.6-4.8-4-5.8V3"/><circle cx="10" cy="15" r="1.3" fill="currentColor"/><circle cx="14.5" cy="12.5" r="1" fill="currentColor"/></svg></span><span>Hakko<small>Supervision fermentation</small></span></a>
    <nav class="nav" id="nav"></nav>
    <div class="side-foot">
      <strong><span class="pulse"></span>Home Assistant</strong>
      <span class="num" id="syncTxt">Synchronisé</span>
    </div>
  </aside>
  <div class="main">
    <header class="topbar">
      <a class="brand-mark m-brand" href="#/" aria-label="Accueil"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M8 3h8M9 3v3.2C6.6 7.2 5 9.4 5 12v6a3 3 0 0 0 3 3h8a3 3 0 0 0 3-3v-6c0-2.6-1.6-4.8-4-5.8V3"/><circle cx="10" cy="15" r="1.3" fill="currentColor"/><circle cx="14.5" cy="12.5" r="1" fill="currentColor"/></svg></a>
      <div class="crumbs" id="crumbs"></div>
      <div class="top-right">
        <span class="sync"><span class="pulse"></span><span class="sync-txt num" id="syncTop">En direct</span></span>
        <button class="icon-btn" data-action="theme" id="themeBtn" aria-label="Changer de thème"></button>
      </div>
    </header>
...
## Annexe B — CSS complet (dans `<style>` du `<head>`)
...
<title>Hakko — Supervision des fermentations</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Sans:wght@400;500;600&display=swap" rel="stylesheet">
<style> /* CSS ci-dessous */ </style>
</head>
```
...
:root{
  ...
  --ok:#15803D; --ok-bg:#E7F6EC; --warn:#B45309; --warn-bg:#FDF3E1; --bad:#B91C1C; --bad-bg:#FDECEC;
  --biere:#C98410; --miso:#95602F; --koji:#5A8A2E; --garum:#9B2C4A;
  --side:#151A21; --side-ink:#AEB6C2; --side-ink2:#F3F5F8; --side-active:#232A34; --side-line:#262D38;
...
@media (prefers-color-scheme:dark){
  :root:not([data-theme="light"]){
    --bg:#0E1116; --surface:#161A21; --surface2:#1A1F27; --hover:#1E242D;
    --ink:#E6EAF0; --ink2:#C4CAD3; --muted:#8C95A3; --faint:#6B7380;
    --line:#262C36; --line2:#323A46; --grid:#1F252E;
    --accent:#7FA2EC; --accent-soft:#1C2740;
    --ok:#5CC486; --ok-bg:#142A1D; --warn:#E8A955; --warn-bg:#2E2412; --bad:#F07F7F; --bad-bg:#321818;
    --biere:#E3A53A; --miso:#C98E5A; --koji:#8DBE5C; --garum:#D66E8A;
    --side:#0A0C10; --side-active:#1B2029; --side-line:#1C212A;
...
body{margin:0;background:var(--bg);color:var(--ink);font-family:'IBM Plex Sans',ui-sans-serif,system-ui,-apple-system,'Segoe UI',Roboto,sans-serif;font-size:14px;line-height:1.5;-webkit-font-smoothing:antialiased}
...
.side{position:sticky;top:0;align-self:start;height:100vh;background:var(--side);color:var(--side-ink);display:flex;flex-direction:column;border-right:1px solid var(--side-line)}
.brand{display:flex;align-items:center;gap:10px;height:60px;padding:0 20px;color:var(--side-ink2);font-weight:600;font-size:15px;border-bottom:1px solid var(--side-line)}
.brand-mark{width:28px;height:28px;border-radius:7px;background:#F3F5F8;color:#151A21;display:grid;place-items:center}
.brand-mark svg{width:18px;height:18px}
.brand small{display:block;font-weight:400;font-size:12px;color:var(--side-ink);margin-top:-2px}
.nav{padding:16px 12px;display:flex;flex-direction:column;gap:2px}
.nav-sec{font-size:12px;color:#6F7885;padding:14px 10px 6px}
.nav-sec:first-child{padding-top:0}
.nav a{display:flex;align-items:center;gap:10px;height:36px;padding:0 10px;border-radius:6px;font-weight:500}
.nav a:hover{color:var(--side-ink2);background:rgba(255,255,255,.04)}
.nav a[aria-current="page"]{background:var(--side-active);color:var(--side-ink2)}
.nav a[aria-current="page"] svg{color:#8FB0F5}
.nav a .cnt{margin-left:auto;font-size:12px;color:#6F7885}
.side-foot{margin-top:auto;padding:14px 20px;border-top:1px solid var(--side-line);font-size:12.5px;display:flex;flex-direction:column;gap:4px}
.side-foot strong{color:var(--side-ink2);font-weight:500;display:flex;align-items:center;gap:8px}
.pulse{width:8px;height:8px;border-radius:50%;background:#34C06A;box-shadow:0 0 0 3px rgba(52,192,106,.18)}
.main{min-width:0;display:flex;flex-direction:column}
.topbar{position:sticky;top:0;z-index:10;height:60px;display:flex;align-items:center;gap:16px;padding:0 32px;background:color-mix(in srgb,var(--surface) 92%,transparent);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);border-bottom:1px solid var(--line)}
.crumbs{display:flex;align-items:center;gap:8px;color:var(--muted);min-width:0;white-space:nowrap;overflow:hidden}
.crumbs a:hover{color:var(--ink)}
.crumbs .sep{color:var(--line2)}
.crumbs .cur{color:var(--ink);font-weight:500;overflow:hidden;text-overflow:ellipsis}
.top-right{margin-left:auto;display:flex;align-items:center;gap:14px;color:var(--muted);font-size:13px}
.sync{display:flex;align-items:center;gap:8px}
.icon-btn{width:34px;height:34px;display:grid;place-items:center;border:1px solid var(--line);background:var(--surface);border-radius:7px;cursor:pointer;color:var(--ink2)}
.icon-btn:hover{background:var(--hover)}
.icon-btn.bare{border-color:transparent;background:none}
.content{padding:28px 32px 64px;max-width:1440px;width:100%}
.tabbar{display:none}
...
@media (max-width:900px){
  ...
  .m-brand{display:grid}
  .sync-txt{display:none}
  .content{padding:20px 16px calc(84px + env(safe-area-inset-bottom,0px))}
  .tabbar{display:flex;position:fixed;left:0;right:0;bottom:0;z-index:20;background:var(--surface);border-top:1px solid var(--line);padding:6px 8px calc(6px + env(safe-area-inset-bottom,0px))}
  .tabbar a{flex:1;display:flex;flex-direction:column;align-items:center;gap:2px;padding:6px 0;color:var(--muted);font-size:11.5px;font-weight:500}
  .tabbar a svg{width:20px;height:20px}
  .tabbar a[aria-current="page"]{color:var(--accent)}
}

/* ---------- Page header ---------- */
.ph{display:flex;justify-content:space-between;align-items:flex-start;gap:16px;margin-bottom:24px;flex-wrap:wrap}
.ph h1{font-size:22px;line-height:1.3}
.ph p{margin:4px 0 0;color:var(--muted)}
.ph-actions{display:flex;gap:8px;flex-wrap:wrap}
.btn{display:inline-flex;align-items:center;justify-content:center;gap:7px;height:36px;padding:0 14px;border:1px solid var(--line2);background:var(--surface);border-radius:7px;cursor:pointer;font-weight:500;white-space:nowrap;box-shadow:var(--shadow);color:var(--ink)}
.btn:hover{background:var(--hover)}
.btn.primary{background:var(--accent);border-color:var(--accent);color:#fff}
:root[data-theme="dark"] .btn.primary{color:#0E1116}
@media (prefers-color-scheme:dark){:root:not([data-theme="light"]) .btn.primary{color:#0E1116}}
.btn.primary:hover{filter:brightness(1.07)}
.btn.ghost{border-color:transparent;background:none;box-shadow:none;color:var(--ink2)}
...
.card-h{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 18px;border-bottom:1px solid var(--line);min-height:52px}
.card-h h2{font-size:14px}
.card-h .sub{color:var(--muted);font-size:13px;font-weight:400;margin-left:6px}
.card-b{padding:18px}
.badge{display:inline-flex;align-items:center;gap:6px;height:22px;padding:0 8px;border-radius:5px;font-size:12px;font-weight:500;background:var(--surface2);color:var(--ink2);border:1px solid var(--line);white-space:nowrap}
.badge.ok{background:var(--ok-bg);color:var(--ok);border-color:transparent}
.badge.warn{background:var(--warn-bg);color:var(--warn);border-color:transparent}
.badge.bad{background:var(--bad-bg);color:var(--bad);border-color:transparent}
.badge .dot{width:6px;height:6px;border-radius:50%;background:currentColor}
.type{display:inline-flex;align-items:center;gap:7px;font-size:13px;color:var(--ink2);white-space:nowrap}
.type i{width:8px;height:8px;border-radius:2px;background:var(--c)}
.muted{color:var(--muted)}
.tabs{display:flex;gap:4px;border-bottom:1px solid var(--line);padding:0 18px}
.tabs button{border:0;background:none;height:44px;padding:0 10px;cursor:pointer;color:var(--muted);font-weight:500;border-bottom:2px solid transparent;margin-bottom:-1px}
.tabs button:hover{color:var(--ink)}
.tabs button[aria-selected="true"]{color:var(--ink);border-bottom-color:var(--accent)}
.tabs .cnt{margin-left:6px;font-size:12px;padding:1px 6px;border-radius:10px;background:var(--surface2);color:var(--muted);border:1px solid var(--line)}
.seg{display:inline-flex;border:1px solid var(--line2);border-radius:7px;overflow:hidden;background:var(--surface)}
.seg button{border:0;background:none;height:30px;padding:0 11px;cursor:pointer;color:var(--muted);font-size:13px;font-weight:500;border-left:1px solid var(--line2)}
...
.bar>div{height:100%;border-radius:3px;background:var(--c,var(--accent))}

/* ---------- KPIs ---------- */
.kpis{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;margin-bottom:20px}
.kpi{padding:16px 18px}
.kpi .k{font-size:13px;color:var(--muted);display:flex;align-items:center;gap:8px}
.kpi .v{font-size:26px;font-weight:600;letter-spacing:-.02em;margin-top:6px;line-height:1.15}
.kpi .v small{font-size:14px;font-weight:500;color:var(--muted);margin-left:3px;letter-spacing:0}
.kpi .s{font-size:12.5px;color:var(--muted);margin-top:4px}
.kpi .s.ok{color:var(--ok)} .kpi .s.bad{color:var(--bad)} .kpi .s.warn{color:var(--warn)}

/* ---------- Overview ---------- */
.ov{display:grid;grid-template-columns:minmax(0,1fr) 340px;gap:20px;align-items:start}
.lots{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:12px;padding:12px}
.lot{display:flex;flex-direction:column;border:1px solid var(--line);border-radius:8px;background:var(--surface);transition:border-color .12s,box-shadow .12s}
.lot:hover{border-color:var(--line2);box-shadow:0 2px 8px rgba(16,24,40,.06)}
.lot-h{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:12px 14px 0}
.lot-n{padding:6px 14px 0;font-weight:600;font-size:15px}
.lot-m{padding:2px 14px 0;color:var(--muted);font-size:12.5px}
.lot-metrics{display:grid;grid-template-columns:repeat(3,1fr);margin:12px 14px 0;border:1px solid var(--line);border-radius:7px}
.lot-metrics>div{padding:8px 10px;border-left:1px solid var(--line)}
.lot-metrics>div:first-child{border-left:0}
.lot-metrics .k{font-size:11.5px;color:var(--muted)}
.lot-metrics .v{font-size:16px;font-weight:600}
.lot-metrics .v small{font-size:12px;color:var(--muted);font-weight:500;margin-left:2px}
.spark{display:block;width:calc(100% - 28px);height:44px;margin:10px 14px 0}
.lot-f{padding:10px 14px 14px;margin-top:auto}
.lot-f .row{display:flex;justify-content:space-between;font-size:12.5px;color:var(--muted);margin-bottom:6px}
.feed{list-style:none;margin:0;padding:6px 0}
.feed li{display:grid;grid-template-columns:28px 1fr;gap:10px;padding:9px 18px}
.feed .fi{width:28px;height:28px;border-radius:7px;display:grid;place-items:center;background:var(--surface2);border:1px solid var(--line);color:var(--muted)}
.feed .fi svg{width:14px;height:14px}
.feed .fi.heat{color:var(--warn)} .feed .fi.note{color:var(--accent)} .feed .fi.batch{color:var(--ok)}
.feed .ft{font-size:13px;color:var(--ink2);line-height:1.4}
.feed .fm{font-size:12px;color:var(--faint)}
.side-stack{display:flex;flex-direction:column;gap:20px}
.health{list-style:none;margin:0;padding:6px 0}
.health li{display:flex;align-items:center;gap:10px;padding:8px 18px;font-size:13px}
.health li .nm{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--ink2)}
.health li .vl{color:var(--muted)}
@media (max-width:1180px){.ov{grid-template-columns:1fr}}

/* ---------- Tables ---------- */
.tbl-wrap{overflow-x:auto}
table.data{width:100%;border-collapse:collapse;font-size:13.5px}
table.data th{text-align:left;font-weight:500;color:var(--muted);font-size:12.5px;padding:10px 16px;background:var(--surface2);border-bottom:1px solid var(--line);white-space:nowrap}
...
table.data tr.click:hover td{background:var(--hover)}
table.data .r{text-align:right}
table.data .nm{font-weight:500;color:var(--ink)}
table.data .sm{font-size:12.5px;color:var(--muted)}
table.data select{height:30px;padding:0 8px;font-size:13px;max-width:210px}
.bat{display:flex;align-items:center;gap:8px}
.bat .bar{width:48px}
.dev-cell{display:flex;align-items:center;gap:10px}
.dev-ic{width:30px;height:30px;border-radius:7px;background:var(--surface2);border:1px solid var(--line);display:grid;place-items:center;color:var(--ink2);flex:none}
.dev-ic svg{width:15px;height:15px}
.ctl{display:flex;align-items:center;gap:8px;justify-content:flex-end}
.switch{width:36px;height:20px;border-radius:999px;border:0;background:var(--line2);position:relative;cursor:pointer;flex:none;transition:background .15s}
...
@media (max-width:900px){
  ...
  table.data.stack tr{padding:12px 16px;border-bottom:1px solid var(--line)}
  table.data.stack tr:last-child{border-bottom:0}
  table.data.stack td{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:4px 0;border:0;text-align:right}
  table.data.stack td[data-l]::before{content:attr(data-l);color:var(--muted);font-size:12.5px;text-align:left}
  table.data.stack td.first{justify-content:flex-start;text-align:left;padding-bottom:8px}
  table.data.stack .ctl{justify-content:flex-end}
}
...
/* ---------- Inputs ---------- */
input[type=text],input[type=number],input[type=search],select,textarea{height:36px;background:var(--surface);border:1px solid var(--line2);border-radius:7px;padding:0 11px;width:100%;outline:none;box-shadow:var(--shadow)}
textarea{height:auto;min-height:88px;padding:9px 11px;resize:vertical;line-height:1.5}
...
.field .unit b{position:absolute;right:11px;top:50%;transform:translateY(-50%);font-weight:400;color:var(--faint);font-size:13px}
.fgrid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}
.fgrid .full{grid-column:1/-1}
@media (max-width:640px){.fgrid{grid-template-columns:1fr}}

/* ---------- Detail ---------- */
.dh{display:flex;justify-content:space-between;gap:16px;flex-wrap:wrap;margin-bottom:20px}
.dh h1{font-size:24px;line-height:1.25;margin-top:8px}
.dh .meta{display:flex;gap:10px;align-items:center;flex-wrap:wrap}
.dh .sub{color:var(--muted);margin-top:4px}
.dh .sub a{color:var(--accent)}
.dgrid{display:grid;grid-template-columns:minmax(0,1fr) 360px;gap:20px;align-items:start;margin-bottom:20px}
.dcol{display:flex;flex-direction:column;gap:20px}
.chart{position:relative;width:100%;touch-action:pan-y}
.chart svg{display:block;overflow:visible}
.chart .tip{position:absolute;top:-4px;pointer-events:none;background:var(--ink);color:var(--surface);font-size:12px;padding:4px 8px;border-radius:5px;white-space:nowrap;opacity:0;transform:translateX(-50%)}
...
.legend i{width:14px;height:2px;background:var(--c)}
.legend i.dash{background:none;border-top:2px dashed var(--c);opacity:.7}
.legend i.band{height:10px;background:var(--c);opacity:.12}
.kv{margin:0;display:grid;grid-template-columns:auto 1fr;font-size:13.5px}
.kv dt{color:var(--muted);padding:9px 0;border-bottom:1px solid var(--line)}
.kv dd{margin:0;text-align:right;padding:9px 0;border-bottom:1px solid var(--line);font-weight:500}
.kv dt:last-of-type,.kv dd:last-of-type{border-bottom:0}
.strip{display:grid;grid-template-columns:repeat(auto-fill,minmax(12px,1fr));gap:3px}
.strip span{aspect-ratio:1;border-radius:2px;background:var(--grid)}
.strip span.past{background:var(--c)}
.strip span.now{background:var(--c);opacity:.45;outline:1.5px solid var(--c);outline-offset:1px}
.prog-top{display:flex;justify-content:space-between;align-items:baseline;margin-bottom:12px}
.prog-top strong{font-size:20px;font-weight:600}
.dates{display:grid;grid-template-columns:repeat(3,1fr);margin-top:16px;border-top:1px solid var(--line);padding-top:12px;font-size:12.5px}
.dates .k{color:var(--muted)} .dates .v{font-weight:500;font-size:13.5px}
.dates div:last-child{text-align:right}
.dates div:nth-child(2){text-align:center}
.d3{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:20px}
.notes{list-style:none;margin:0;padding:0}
.notes li{padding:12px 18px;border-bottom:1px solid var(--line)}
.notes li:last-child{border-bottom:0}
.notes time{display:block;font-size:12px;color:var(--faint);margin-bottom:2px}
.note-form{display:flex;gap:8px;padding:12px 18px;border-top:1px solid var(--line);background:var(--surface2);border-radius:0 0 10px 10px}
.note-form input{flex:1;min-width:0}
.steps{margin:0;padding:0;list-style:none;counter-reset:s}
.steps li{counter-increment:s;display:grid;grid-template-columns:24px 1fr;gap:10px;padding:10px 18px;border-bottom:1px solid var(--line);font-size:13.5px}
.steps li:last-child{border-bottom:0}
.steps li::before{content:counter(s);width:22px;height:22px;border-radius:50%;background:var(--surface2);border:1px solid var(--line);display:grid;place-items:center;font-size:11.5px;color:var(--muted);font-weight:500}
@media (max-width:1180px){.dgrid{grid-template-columns:1fr}.d3{grid-template-columns:1fr 1fr}}
@media (max-width:760px){.d3{grid-template-columns:1fr}}

/* ---------- Recipe form ---------- */
.rf{display:grid;grid-template-columns:minmax(0,1fr) 300px;gap:20px;align-items:start}
.rf .dcol{gap:20px}
.types{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}
.types label{cursor:pointer;position:relative}
.types input{position:absolute;opacity:0;pointer-events:none}
.types span{display:flex;align-items:center;gap:10px;height:44px;padding:0 12px;border:1px solid var(--line2);border-radius:8px;font-weight:500;background:var(--surface)}
.types span i{width:10px;height:10px;border-radius:3px;background:var(--c)}
.types input:checked+span{border-color:var(--accent);box-shadow:0 0 0 3px var(--accent-soft)}
.types input:focus-visible+span{outline:2px solid var(--accent);outline-offset:2px}
.t-only{display:none}
form[data-type="biere"] .t-biere,form[data-type="koji"] .t-koji,form[data-type="miso"] .t-salt,form[data-type="garum"] .t-salt{display:flex}
.rows{display:flex;flex-direction:column}
.rrow{display:grid;grid-template-columns:minmax(0,1fr) 140px 34px;gap:8px;align-items:center;padding:8px 18px;border-bottom:1px solid var(--line)}
.rrow.step{grid-template-columns:24px minmax(0,1fr) 34px}
.rrow .ix{font-size:12.5px;color:var(--muted);text-align:center}
.rows-h{display:grid;grid-template-columns:minmax(0,1fr) 140px 34px;gap:8px;padding:8px 18px;background:var(--surface2);border-bottom:1px solid var(--line);font-size:12.5px;color:var(--muted);font-weight:500}
.add-row{padding:10px 18px}
.sticky{position:sticky;top:80px}
.err{color:var(--bad);font-size:13px;margin:0}
@media (max-width:1100px){.rf{grid-template-columns:1fr}.sticky{position:static}}
@media (max-width:640px){.types{grid-template-columns:1fr 1fr}.rrow,.rows-h{grid-template-columns:minmax(0,1fr) 96px 34px}}

/* ---------- Dialog ---------- */
dialog{border:1px solid var(--line);border-radius:12px;padding:0;background:var(--surface);color:var(--ink);width:min(480px,calc(100vw - 32px));box-shadow:0 20px 50px rgba(0,0,0,.2)}
dialog::backdrop{background:rgba(15,20,28,.45)}
dialog .card-b{display:flex;flex-direction:column;gap:14px}
dialog .foot{display:flex;justify-content:flex-end;gap:8px;padding:14px 18px;border-top:1px solid var(--line);background:var(--surface2);border-radius:0 0 12px 12px}

.d2{display:grid;grid-template-columns:minmax(0,1.6fr) minmax(0,1fr);gap:20px;align-items:start}
@media (max-width:1100px){.d2{grid-template-columns:1fr}}
.journal{max-height:460px;overflow-y:auto}
.journal li.jl{display:flex;flex-direction:column;gap:3px}
.jrow{display:flex;align-items:baseline;gap:10px;flex-wrap:wrap}
.jrow .badge{flex:none;min-width:78px;justify-content:center}
.jrow strong{font-weight:600}
.badge.acc{background:var(--accent-soft);color:var(--accent);border-color:transparent}
.tset{display:flex;gap:10px;align-items:center;flex-wrap:wrap}
.stepper{display:flex;align-items:center;border:1px solid var(--line2);border-radius:7px;overflow:hidden;background:var(--surface);flex:1;min-width:180px;box-shadow:var(--shadow)}
.stepper button{width:40px;height:40px;border:0;background:var(--surface2);cursor:pointer;font-size:18px;color:var(--ink2);flex:none}
.stepper button:hover{background:var(--hover)}
.stepper input{border:0;box-shadow:none;height:40px;text-align:center;font-size:18px;font-weight:600;border-radius:0;-moz-appearance:textfield;min-width:0}
.stepper input::-webkit-inner-spin-button,.stepper input::-webkit-outer-spin-button{-webkit-appearance:none;margin:0}
.stepper input:focus{box-shadow:none}
.stepper span{color:var(--muted);padding-right:10px}
.hint{margin:10px 0 0;font-size:12.5px;color:var(--muted)}
.grav-form{display:grid;grid-template-columns:1fr 1.3fr auto;gap:10px;align-items:end;margin-top:18px;padding-top:16px;border-top:1px solid var(--line)}
input[type=datetime-local]{height:36px;background:var(--surface);border:1px solid var(--line2);border-radius:7px;padding:0 11px;width:100%;outline:none;box-shadow:var(--shadow)}
@media (max-width:640px){.grav-form{grid-template-columns:1fr}}

/* ---------- Detail: single screen ---------- */
.dp-head{display:flex;justify-content:space-between;align-items:center;gap:16px;flex-wrap:wrap;margin-bottom:14px}
.dp-title{display:flex;align-items:center;gap:12px;flex-wrap:wrap;min-width:0}
.dp-title h1{font-size:21px;line-height:1.2}
.dp-sub{color:var(--muted);font-size:13px}
.kpis.compact{grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin-bottom:14px}
.kpis.compact .kpi{padding:10px 14px}
.kpis.compact .kpi .v{font-size:20px;margin-top:2px}
.kpis.compact .kpi .s{margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.dp-grid{display:grid;grid-template-columns:minmax(0,1.35fr) minmax(0,1fr) 320px;grid-template-rows:minmax(0,1.2fr) minmax(0,1fr);gap:14px}
.dp-chart{grid-column:1/3;grid-row:1;display:flex;flex-direction:column;min-height:0}
.dp-journal{grid-column:1;grid-row:2}
.dp-devs{grid-column:2;grid-row:2}
.dp-side{grid-column:3;grid-row:1/3;display:flex;flex-direction:column;gap:14px;min-height:0}
.dp-side>.card{flex:none}
.dp-side>.dp-params{flex:1 1 auto}
.tabs{align-items:center}
.tabs-end{margin-left:auto}
.dp-chart-body{flex:1;min-height:0;display:flex;gap:16px;padding:12px 16px 12px}
.dp-plot{flex:1;min-width:0;display:flex;flex-direction:column;min-height:0}
.chart.fill{flex:1;min-height:0}
.chart.fill svg{position:absolute;inset:0}
.dp-plot .legend{margin-top:6px}
.grav-side{width:300px;flex:none;display:flex;flex-direction:column;min-height:0;border-left:1px solid var(--line);padding-left:16px}
.grav-side .grav-form{grid-template-columns:1fr;margin:0;padding:0;border:0;gap:8px}
.grav-list{flex:1;min-height:0;margin-top:12px;border:1px solid var(--line);border-radius:8px}
.grav-list table.data th{position:sticky;top:0;z-index:1}
.grav-list table.data td,.grav-list table.data th{padding:7px 10px}
.scroll-card{display:flex;flex-direction:column;min-height:0;overflow:hidden}
.scroll-card>.card-h{flex:none}
.scroll{flex:1 1 auto;min-height:0;overflow-y:auto;scrollbar-width:thin;scrollbar-color:var(--line2) transparent}
.scroll::-webkit-scrollbar{width:8px}
.scroll::-webkit-scrollbar-thumb{background:var(--line2);border-radius:8px;border:2px solid var(--surface)}
.scroll::-webkit-scrollbar-track{background:transparent}
.journal.scroll{max-height:none}
.journal li.jl{display:grid;grid-template-columns:104px 88px minmax(0,1fr);align-items:center;gap:10px;padding:8px 16px;flex-direction:row}
.journal li.jl time{margin:0;font-size:12px}
.journal li.jl .badge{justify-content:center}
.journal li.jl .jt{font-size:13px;color:var(--ink2);line-height:1.4}
.journal li.jl .jt strong{color:var(--ink)}
.journal li.muted{padding:14px 16px}
.dp-journal .card-h,.dp-devs .card-h,.dp-side .card-h{padding:10px 16px;min-height:46px}
.dp-journal .note-form{flex:none;padding:10px 16px}
.dp-side .card-b{padding:12px 16px}
.dp-side .kv{padding:0 16px}
.dp-side .kv dt,.dp-side .kv dd{padding:7px 0}
.dp-side .prog-top{margin-bottom:10px}
.dp-side .prog-top strong{font-size:17px}
.dp-side .dates{margin-top:12px;padding-top:10px}
.dp-side .stepper button,.dp-side .stepper input{height:36px}
.dp-side .hint{margin-top:8px}
.dp-devs table.data td{padding:10px 16px}
@media (min-width:1101px) and (min-height:640px){
  .content[data-page="detail"]{padding-top:20px;padding-bottom:20px}
  ...
  .dp-grid{flex:1;min-height:0}
}
@media (max-width:1100px){
  .dp-grid{grid-template-columns:1fr;grid-template-rows:none}
  .dp-chart,.dp-journal,.dp-devs,.dp-side{grid-column:auto;grid-row:auto}
  .dp-chart{height:380px}
  .dp-journal{height:420px}
  .dp-side>.dp-params{flex:none}
  .dp-chart-body.with-side{flex-direction:column}
  .dp-chart.dp-chart:has(.with-side){height:auto}
  .dp-chart-body.with-side .dp-plot{height:260px;flex:none}
  .grav-side{width:auto;border-left:0;padding-left:0}
  .grav-list{max-height:240px}
}
@media (max-width:640px){
  .journal li.jl{grid-template-columns:auto 1fr;row-gap:4px}
  .journal li.jl time{grid-column:1/-1}
}

.dp-grid{grid-template-columns:minmax(0,1.7fr) minmax(0,1fr) 320px;grid-template-rows:minmax(0,1.3fr) minmax(0,1fr)}
.dp-chart .tabs button{height:42px}
.dp-side .tset{flex-wrap:nowrap}
.dp-side .stepper{min-width:0}
.dp-side .strip{grid-template-columns:repeat(auto-fill,minmax(10px,1fr))}
.journal li.jl{grid-template-columns:92px 86px minmax(0,1fr)}
.grav-side{width:340px}
.grav-side .grav-form{grid-template-columns:110px minmax(0,1fr)}
.grav-side .grav-form .btn{grid-column:1/-1}
.grav-side .field>span{font-size:12.5px}
.grav-list td,.grav-list th{white-space:nowrap}
@media (max-width:1100px){.dp-grid{grid-template-columns:1fr;grid-template-rows:none}.grav-side{width:auto}}

.journal li.jl{grid-template-columns:84px 80px minmax(0,1fr);gap:8px}
.journal li.jl .badge{font-size:11.5px;padding:0 6px}
.dp-devs td.r{white-space:nowrap}
.journal li.jl{grid-template-columns:auto 80px minmax(0,1fr)}
.journal li.jl time{white-space:nowrap}

.arch-desc{margin:12px 16px 4px;color:var(--muted);font-size:13.5px}
.arch-sub{padding:12px 16px 4px;font-size:12.5px;font-weight:600;color:var(--muted);border-top:1px solid var(--line);margin-top:6px}
.dp-params .steps li{padding:8px 16px}
.dp-grid.arch .dp-chart-body{padding:10px 16px}
.dp-grid.arch .arch-2{overflow:visible}
@media (max-width:1100px){.dp-grid.arch .arch-2{height:320px}.dp-grid.arch .dp-devs{height:360px}}
.tabbar a span{white-space:nowrap}

.dp-grid.arch .journal li.jl{grid-template-columns:auto 1fr;row-gap:2px}
.dp-grid.arch .journal li.jl .jt{grid-column:1/-1}
.dp-grid.arch .journal li.jl .badge{justify-self:end}
.search input[type=search]{padding-left:34px}

/* ---------- Multiprise ---------- */
.strip-h{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 18px;border-bottom:1px solid var(--line)}
.strip-title{display:flex;align-items:center;gap:12px}
.strip-title>svg{width:34px;height:34px;padding:8px;border-radius:8px;background:var(--surface2);border:1px solid var(--line);color:var(--ink2)}
.strip-title .nm{font-weight:600}
.strip-title .sm{font-size:12.5px;color:var(--muted)}
.outlets{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:12px;padding:16px 18px 18px}
.outlet{display:flex;flex-direction:column;gap:10px;padding:14px;border:1px solid var(--line);border-radius:10px;background:var(--surface2);transition:border-color .15s}
.outlet.is-on{border-color:color-mix(in srgb,var(--ok) 55%,var(--line))}
.outlet.off{opacity:.55}
.o-top{display:flex;justify-content:space-between;align-items:center;font-size:12.5px;color:var(--muted)}
.o-slot{display:inline-flex;align-items:center;gap:7px;font-weight:600;color:var(--ink2)}
.o-led{width:8px;height:8px;border-radius:50%;background:var(--bad);box-shadow:0 0 0 3px color-mix(in srgb,var(--bad) 18%,transparent)}
.outlet.is-on .o-led{background:var(--ok);box-shadow:0 0 0 3px color-mix(in srgb,var(--ok) 22%,transparent)}
.o-pw{font-weight:500;color:var(--ink2)}
.o-name{font-weight:600;font-size:14.5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.o-f{display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)}
.o-f select{height:32px;font-size:13px;padding:0 8px}
.o-btn{margin-top:auto;height:44px;border:0;border-radius:8px;cursor:pointer;font-weight:700;font-size:15px;letter-spacing:.06em;color:#fff;background:#C53030;transition:background .15s,transform .05s}
.o-btn:hover{filter:brightness(1.08)}
.o-btn:active{transform:scale(.98)}
.o-btn[aria-pressed="true"]{background:#1F9D55}
.o-btn:disabled{cursor:not-allowed;filter:grayscale(.6)}
@media (max-width:1250px){.outlets{grid-template-columns:repeat(3,minmax(0,1fr))}}
@media (max-width:760px){.outlets{grid-template-columns:repeat(2,minmax(0,1fr))}}
@media (max-width:440px){.outlets{grid-template-columns:1fr}}

/* ---------- Home: single screen ---------- */
.home-grid{grid-template-columns:minmax(0,1fr) 340px;gap:14px}
.home .ov{align-items:stretch}
.home .side-stack{gap:14px;min-height:0}
.home .feed-card{flex:1 1 60%}
.home .health-card{flex:1 1 40%}
.home .card-h{padding:10px 16px;min-height:46px}
.home .lots{align-content:start;gap:12px;padding:12px}
.home .lot-h{padding:10px 12px 0}
.home .lot-n{padding:4px 12px 0;font-size:14.5px}
.home .lot-m{padding:0 12px}
.home .lot-metrics{margin:8px 12px 0}
.home .lot-metrics>div{padding:6px 10px}
.home .lot-metrics .v{font-size:15px}
.home .spark{height:34px;margin:8px 12px 0;width:calc(100% - 24px)}
.home .lot-f{padding:8px 12px 12px}
.home .feed li{padding:7px 16px}
.home .health li{padding:7px 16px}
@media (min-width:1101px) and (min-height:640px){
  .home.dp .home-grid{flex:1;min-height:0}
}
@media (max-width:1180px){.home-grid{grid-template-columns:1fr}.home .feed-card{max-height:360px}.home .lots.scroll{overflow:visible}}
.home .lots{grid-template-columns:repeat(auto-fill,minmax(250px,1fr))}
@media (max-width:1360px){.home-grid{grid-template-columns:minmax(0,1fr) 300px}}
```

## Annexe C — JavaScript complet (dans `<script>` juste avant `</body>`)
